...
*/
public class TrafficController extends JFrame implements CommandReceiver, Runnable, InterpreterInterface
{
...
//				ip_log = new JFrame("IP?ｿｽ?ｽｽA?ｿｽ?ｽｽh?ｿｽ?ｽｽ?ｿｽ?ｽｽ?ｿｽ?ｽｽX?ｿｽ?ｽｽA?ｿｽ?ｽｽ|?ｿｽ?ｽｽ[?ｿｽ?ｽｽg?ｿｽ?ｽｽ?ｾ丞･?ｽｿ?ｽｽ?ｿｽ?ｽｽ?ｾ檎｢ｺ?ｿｽ?ｽｽF");
			    {
...
//					mainController.parseCommand("twitterConnect", "");
					parseApplicationCommand("pjc","twitterConnect","");
...
//				mainController.parseCommand("twitterTweet", x);
				parseApplicationCommand("pjc","twitterTweet",x);
	...
	public String command(String x, String v) {
		...
		if(x.startsWith("set ")){
			...
			if(x2.equals("readInterval")){
				...
				this.saveProperties();
				*/
				return this.parseApplicationCommand("ptGui", x, v);
			}
			else
...
//			String rtn=this.wikiUrlTextField.getText();
			String rtn=this.parseApplicationCommand("ptGui",x, v);
			return rtn;
		}
		else
...
//	   	service.parseCommand("guiMessage",x);
			...
			if(Util.parseKeyWord(l, "repeating ", rest)) {
				...
				if(Util.parseKeyWord(l, "over ", rest)) {
					...
					if(Util.parseInt(l, num, rest)) {
						this.getRepeatingTrafficOver(num[0],bfi);
						return "OK";
					}
				}
			}
	...
	private String parseApplicationCommand(String appliName, String x, String y) {
	...
	private void getHostsInThisLan() {
		if(this.hostsInformation==null) return;
		DateFormat df=new SimpleDateFormat("yyy/MM/dd HH:mm:ss Z");
		for(String macAddr: this.hostsInformation.keySet() ) {
			EachHostInformation e=this.hostsInformation.get(macAddr);
			if(e!=null) {
				String date=df.format(new Date());
			   parseApplicationCommand("pjc", "wikiPutSendBuffer","cmd=get hosts, date=\""+date+"\", nif="+e.nif+", mac=\""+macAddr+"\", ip=\""+e.getIPs()+"\".\n");
			}
		}
	}
	private void getDomainList() {
        ...
        for(String key: domainList.keySet()) {
     	   if(key!=null) {
     		   ...
     		   String dnsLine="cmd=get domain-list, date="+time+", qhost=\""+qhost+"\", name="+key+", ip="+aip+", server="+server+".\n";
	...
	private void writeDhcpList(TrafficHistory dhcpH) {
        if(dhcpH==null) return;
        ...
        for(int i=0;i<dhcpList.size();i++) {
               ...
               String option=st.nextToken();
     		   String dhcpLine="cmd=get dhcp-list, date=\""+time+
     				   "\", nif="+nif+
		...
		for(String macAddr: this.hostsInformation.keySet() ) {
			...
			if(e!=null) {
			   ...
			   for(int i=0;i<historyNumber-1;i++) {
				  ...
				  long time1=traffic1.getTime();
				  String destIP1=traffic1.getDestinationIP();
				  String destIPNQ=this.deleteDoubleQuotations(destIP1);
				  if(f.isTrue(destIPNQ)) {
				    String sourceIP1=traffic1.getSourceIP();
				    String key=sourceIP1+"-"+destIP1;
				    TimeAndTraffic tt=previousHosts.get(key);
				    if(tt!=null) continue;
				    previousHosts.put(key, traffic1);
				    boolean repeated=false;
				    String repeatedTraffic=traffic1.traffic;
				    boolean firstTime=true;
				    int listCount=0;
				    for(int j=i+1; j<historyNumber;j++) {
					  TimeAndTraffic traffic2=e.history.elementAt(j);
					  String destIP2=traffic2.getDestinationIP();
					  String sourceIP2=traffic2.getSourceIP();
					  if(destIP1.equals(destIP2) && sourceIP1.equals(sourceIP2)) {
						 long time2=traffic2.getTime();						 
						 if(time2-time1>x) {
							 if(firstTime) {
							    parseApplicationCommand("pjc","wikiPutSendBuffer","cmd=get repeating, "+repeatedTraffic+".\n");
							    firstTime=false;
							 }
							 parseApplicationCommand("pjc","wikiPutSendBuffer"," cmd=get repeating, "+traffic2.traffic+".\n");
							 time1=time2;
							 if(listCount>maxRepeatingNumber) break;
							 listCount++;
				  ...
				  /*
				  if(repeated) {
					 parseApplicationCommand("pjc", "wikiPutSendBuffer",repeatedTraffic+".\n");	
				  }
				  */
			   }
