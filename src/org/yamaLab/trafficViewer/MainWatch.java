...
import org.yamaLab.trafficController.CommandTableManager;
...
//	public JTextField messageField;
	...
	long showEndTime;
	int termBarMax=600;
	private TrafficLogManager logManager;
	
	public MainWatch(){
		super();
	}
	public MainWatch(TrafficController m, PacketMonitorFilterInterface f){
		main=m;
		this.initGUI();
		logManager = new TrafficLogManager();		
		logManager.setMainWatch(this);
		logManager.setMonitorFilter(f);
		logManager.setMain(m);
		logManager.start();
	}
	private void initGUI(){
		setTitle("Traffic Viewer   Yamanoue Laboratory, Fukuyama University, Kagoshima University");
...
//
        ...
        */
		...
		jmProtocol.add(allV4Protocol);
		udp.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent evt) {
				allV4ProtocolMouseClicked(evt);
			}
		});
		...
		jmProtocol.add(allV6Protocol);
		udp.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent evt) {
				allV6ProtocolMouseClicked(evt);
			}
		});
...
//				tenSecTermMouseClicked(evt);
		...
		basepanel.add(scrollpanel);
		scrollpanel.setBounds(5, 59, 1051, 650);
		{
...
//    	System.out.println("all1.mouseClicked, event="+evt);
    ...
    private void startButtonActionPerformed(ActionEvent evt) {
		...
		this.main.command("wikiStartWatching", "");
    }
    ...
    private void watchingButtonActionPerformed(ActionEvent evt) {
    	...
    	if(this.watchingButton.isSelected()){
    		if(this.main!=null){
    			CommandTableManager com=this.main.commandTableManager;
    			com.start();
...
//    			com.onlineCommandRefreshButton.setSelected(false);
    ...
    public TrafficLogManager getTrafficLogManager() {
